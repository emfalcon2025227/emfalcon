...
export interface DataContextType {
  ...
  rejectLeaseModification: (leaseId: string, rejectionReason: string) => { success: boolean; lease?: Lease; error?: string };
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { currentUser, hasPermission, users, importUsersBatch } = useAuth();
  const { t } = useLanguage();
  ...
  const mergePaymentAllocations = (existing: PaymentAllocation[], incoming: PaymentAllocation[]): PaymentAllocation[] => {
    const existingIds = new Set(existing.map((a) => a.id));
    const newUnique = incoming.filter((a) => a && a.id && !existingIds.has(a.id));
    return [...newUnique, ...existing];
  };

  const [paymentAllocations, setPaymentAllocations] = useState<PaymentAllocation[]>(() => {
    const raw = safeLoadFromStorage("ef_payment_allocations_v12", []);
    const seen = new Set<string>();
    return (raw || []).filter((a: PaymentAllocation) => {
  ...
  // Real-time Firestore Sync across devices and users (active only when authenticated)
  useEffect(() => {
    if (!currentUser) {
      return;
    }
    ...
    const unsubAllocations = onSnapshot(collection(db, "payment_allocations"), (snap) => {
      if (!snap.empty) {
        ...
        const seen = new Set<string>();
        snap.forEach(d => {
          const item = d.data() as PaymentAllocation;
  ...
  const addLease = (data: Omit<Lease, "id" | "createdAt">): Lease => {
    ...
    // CENTRAL CONTRACT LIFECYCLE ENFORCEMENT:
    // Every new lease contract MUST start as "BINDING".
    // Even if caller sends contractStatus: "ACTIVE", override it to "BINDING".
    ...
    safeSetDoc(doc(db, "leases", newLease.id), sanitizeForFirestore(newLease));
    // CRITICAL: NO FINANCIAL SIDE EFFECTS BEFORE APPROVAL
    // Staged expenses in newLease.leaseExpenses remain preserved on the lease and will only materialize upon approval.
  ...
  const updateLease = (id: string, patch: Partial<Lease>) => {
    ...
    if (
      lease &&
      (lease.contractStatus === "BINDING" || lease.contractStatus === "PENDING_APPROVAL" || lease.contractStatus === "PENDING_AUDIT" || lease.contractStatus === "DRAFT" || lease.contractStatus === "PENDING") &&
      patch.contractStatus === "ACTIVE"
    ) {
      const err = language === "ar"
        ? "خطأ في حوكمة العقود: لا يمكن تحويل العقد إلى ساري (ACTIVE) مباشرة بدون الاعتماد الإداري الصريح من مركز طلبات الاعتماد."
        : "Contract governance error: A contract cannot be set to ACTIVE directly without explicit approval.";
      alert(err);
      throw new Error(err);
    }
    if (lease) {
  ...
  const renewLease = (leaseId: string, newEndDate: string, annualRent: number, overrideReason?: string) => {
    ...
    const own = owners.find((o) => o.id === lease.ownerId);

    // Route through authoritative createLeaseRenewal (PENDING_APPROVAL)
    createLeaseRenewal({
      originalLeaseId: lease.id,
      originalLeaseNumber: lease.leaseNumber,
      ownerId: lease.ownerId,
      ownerNameAr: own?.nameAr,
      ownerNameEn: own?.nameEn,
      propertyId: lease.propertyId,
      propertyNameAr: prop?.nameAr,
      propertyNameEn: prop?.nameEn,
      unitId: lease.unitId,
      unitNumber: unt?.unitNumber,
      tenantId: lease.tenantId,
      tenantNameAr: tenant?.nameAr,
      tenantNameEn: tenant?.nameEn,
      currentAnnualRent: lease.annualRent,
      newAnnualRent: annualRent,
      increaseAmount: annualRent - lease.annualRent,
      increasePercentage: lease.annualRent > 0 ? ((annualRent - lease.annualRent) / lease.annualRent) * 100 : 0,
      originalStartDate: lease.startDate,
      originalEndDate: lease.endDate,
      newStartDate: lease.endDate,
      newEndDate: newEndDate,
      installmentsCount: lease.installmentsCount || 4,
      paymentFrequency: lease.paymentFrequency || "QUARTERLY_4_CHEQUES",
      paymentSchedule: [],
      notes: overrideReason ? `[تجاوز إداري]: ${overrideReason}` : undefined,
    });
    logAudit("CREATE", "LEASE", leaseId, lease.leaseNumber, `Submitted lease renewal request for lease #${lease.leaseNumber} pending approval`);
  ...
  // ============================================================================
  // CONTRACT LIFECYCLE & APPROVAL GATE IMPLEMENTATION
  // ============================================================================

  const submitLeaseForApproval = (
    leaseId: string
  ): { success: boolean; lease?: Lease; error?: string } => {
    const target = leases.find((l) => l.id === leaseId);
    if (!target) return { success: false, error: language === "ar" ? "العقد غير موجود" : "Lease not found" };
    if (target.contractStatus === "ACTIVE") {
      return { success: false, error: language === "ar" ? "العقد ساري بالفعل" : "Lease is already active" };
    }
    const nowIso = new Date().toISOString();
    const updated: Lease = {
      ...target,
    ...
    safeSetDoc(doc(db, "leases", leaseId), sanitizeForFirestore(updated), { merge: true });

    addNotification({
      channel: "PORTAL",
      recipient: "Manager",
      recipientName: language === "ar" ? "إدارة العقود والاعتمادات" : "Contracts Management",
      status: "SENT",
      content: language === "ar"
        ? `📋 تم إرسال عقد الإيجار رقم (${target.leaseNumber}) للاعتماد النهائي وتفعيل الأثر المالي.`
        : `📋 Lease #${target.leaseNumber} submitted for approval.`,
      sentAt: nowIso,
    });

    logAudit("STATUS_CHANGE", "LEASE", target.id, target.leaseNumber, `Submitted lease ${target.leaseNumber} for approval`);
  ...
  const rejectLease = (
    ...
    if (!isAuthorized) {
      return {
        success: false,
        error: language === "ar" ? "غير مصرح لك برفض العقود." : "Unauthorized to reject leases.",
      };
    }
    const lease = leases.find((l) => l.id === leaseId);
    if (!lease) return { success: false, error: language === "ar" ? "العقد غير موجود" : "Lease not found" };
    if (lease.contractStatus === "ACTIVE") {
      return { success: false, error: language === "ar" ? "لا يمكن رفض عقد ساري ونشط بالفعل" : "Cannot reject an active lease" };
    }
    const updated: Lease = {
      ...lease,
    ...
    // Release unit occupancy if no other occupying lease exists
    if (lease.unitId) {
      const otherOccupying = leases.find(
        (l) => l.id !== lease.id && l.unitId === lease.unitId && isOccupyingLeaseStatus(l.contractStatus)
      );
      if (!otherOccupying) {
        setUnits((prevUnits) =>
          prevUnits.map((u) => {
    ...
    logAudit("STATUS_CHANGE", "LEASE", lease.id, lease.leaseNumber, `Rejected lease contract ${lease.leaseNumber}. Reason: ${rejectionReason}`);
    return { success: true, lease: updated };
  };

  const requestLeaseModification = (
    leaseId: string,
    requestData: Omit<LeaseModificationRequest, "id" | "leaseId" | "leaseNumber" | "requestedAt" | "requestedBy" | "status">
  ): { success: boolean; lease?: Lease; error?: string } => {
    const lease = leases.find((l) => l.id === leaseId);
    if (!lease) return { success: false, error: language === "ar" ? "العقد غير موجود" : "Lease not found" };
    const nowIso = new Date().toISOString();
    const userId = currentUser?.id || "sys-01";
    const userName = currentUser?.nameAr || currentUser?.nameEn || "المستخدم";

    const modRequest: LeaseModificationRequest = {
      ...requestData,
      id: "mod-" + Date.now() + "-" + crypto.randomUUID().split("-")[0],
      leaseId: lease.id,
      leaseNumber: lease.leaseNumber,
      requestedAt: nowIso,
    ...
    const updated: Lease = {
      ...lease,
      pendingModification: modRequest,
    };
    setLeases((prev) => prev.map((l) => (l.id === leaseId ? updated : l)));
    safeSetDoc(doc(db, "leases", lease.id), sanitizeForFirestore(updated), { merge: true });
    ...
    addNotification({
      channel: "PORTAL",
      recipient: "Manager",
      recipientName: language === "ar" ? "مدير النظام" : "System Administrator",
      status: "SENT",
      content: language === "ar"
        ? `📝 تم تقديم طلب تعديل مالي لعقد الإيجار رقم (${lease.leaseNumber}) بانتظار الاعتماد الإداري.`
        : `📝 Financial modification requested for Lease #${lease.leaseNumber} pending approval.`,
      sentAt: nowIso,
    });

    logAudit("UPDATE", "LEASE", lease.id, lease.leaseNumber, `Requested modification for active lease ${lease.leaseNumber}`);
    return { success: true, lease: updated };
  };

  const approveLeaseModification = (
    leaseId: string,
    reviewNotes?: string
  ): { success: boolean; lease?: Lease; error?: string } => {
    const isAuthorized =
    ...
    if (!isAuthorized) {
      return {
        success: false,
        error: language === "ar" ? "غير مصرح لك باعتماد تعديلات العقود." : "Unauthorized to approve modifications.",
      };
    }
    const lease = leases.find((l) => l.id === leaseId);
    if (!lease || !lease.pendingModification) {
      return { success: false, error: language === "ar" ? "لا يوجد طلب تعديل معلق لهذا العقد" : "No pending modification found for this lease" };
    }
    if (lease.pendingModification.status !== "PENDING_APPROVAL") {
      return {
        success: false,
        error: language === "ar"
          ? `لا يمكن اعتماد التعديل إلا إذا كان قيد الاعتماد (PENDING_APPROVAL). الحالة الحالية: ${lease.pendingModification.status}`
          : `Modification must be in PENDING_APPROVAL status. Found: ${lease.pendingModification.status}`,
      };
    }
  ...
  const rejectLeaseModification = (
    ...
    if (!isAuthorized) {
      return {
        success: false,
        error: language === "ar" ? "غير مصرح لك برفض تعديلات العقود." : "Unauthorized to reject modifications.",
      };
    }
    const lease = leases.find((l) => l.id === leaseId);
    if (!lease || !lease.pendingModification) {
    ...
    logAudit("STATUS_CHANGE", "LEASE", lease.id, lease.leaseNumber, `Rejected modification for lease ${lease.leaseNumber}. Reason: ${rejectionReason}`);
    return { success: true, lease: updated };
  };
  ...
  const processUnifiedPayment = async (params: {
    ...
    setCollections((prev) => [receipt, ...prev]);
    setPaymentAllocations((prev) => mergePaymentAllocations(prev, createdAllocations));
  ...
  const depositCheque = async (params: {
    ...
    try {
      const result = await runTransaction(db, async (transaction) => {
        ...
        if (!params.depositProofUrl && !params.depositSlipNumber?.trim()) {
          throw new Error(
            language === "ar"
              ? "يجب إرفاق إثبات الإيداع أو إدخال رقم الحافظة."
              : "Deposit proof document or deposit slip number is required."
          );
        }
        const effectiveSlipNumber =
          params.depositSlipNumber?.trim() ||
          target.depositSlipNumber?.trim() ||
          (params.notes?.trim() ? `REF-${params.notes.trim().slice(0, 20)}` : undefined) ||
          `DEP-${target.chequeNumber || Date.now().toString().slice(-6)}`;
        const oldStatus = target.status;
        const nowIso = new Date().toISOString();
        const auditEntry: ChequeAuditEntry = {
          id: "aud-" + Date.now(),
        ...
        const updated: Cheque = {
          ...
          depositSlipNumber: effectiveSlipNumber || target.depositSlipNumber,
  ...
  const clearCheque = async (params: {
    ...
    try {
      ...
      setPaymentAllocations((prev) => mergePaymentAllocations(prev, [result.alloc]));
  ...
  const extractDocumentOCR = async (documentType: string, imageBase64: string, mimeType = "image/jpeg"): Promise<any> => {
    try {
      ...
      if (documentType === "EMIRATES_ID") {
        ...
        if (res.isTransportFailure || (res as any).isHtmlResponse || res.errorType === "TRANSPORT_FAILURE") {
          return {
            success: false,
            isTransportFailure: true,
            errorType: "TRANSPORT_FAILURE",
            error: res.error || "Transport failure connecting to central OCR engine.",
            errorAr: res.errorAr || "تعذر الاتصال بخدمة OCR المركزية. تم إيقاف المعالجة لأن استجابة الخدمة لم تكن JSON صحيحة.",
          };
        }
        ...
        try {
          ...
          if (v2Res.isTransportFailure || v2Res.errorType === "TRANSPORT_FAILURE" || (v2Res.diagnostics && v2Res.diagnostics.errorCode === "TRANSPORT_FAILURE")) {
            return {
              success: false,
              isTransportFailure: true,
              errorType: "TRANSPORT_FAILURE",
              error: v2Res.diagnostics?.errorMsg || "Transport failure connecting to central OCR engine.",
          ...
          console.warn("[DataContext] OCR V2 fallback for Emirates ID:", v2Err);
        }
        return {
          success: false,
  ...
  const collectAdministrativeFee = async (
    ...
    try {
      ...
      setPaymentAllocations((prev) => mergePaymentAllocations(prev, [allocObj]));
  ...
  const liquidateUnallocatedAdvance = async (params: {
    ...
    setPaymentAllocations((prev) => mergePaymentAllocations(prev, newAllocations));
    logAudit(
      "ADVANCE_LIQUIDATION",
      "LEASE",
      leaseObj.id,
  ...
  const importDatabaseJSON = (jsonStr: string): boolean => {
    try {
      ...
      if (parsed.users && Array.isArray(parsed.users) && importUsersBatch) {
        importUsersBatch(parsed.users);
      }
      if (parsed.userProfiles && Array.isArray(parsed.userProfiles) && importUsersBatch) {
        importUsersBatch(parsed.userProfiles);
      }
      ...
      logAudit("DATA_IMPORT", "RISK_CONFIG", "db-import", "Database Transfer", "Successfully imported database state and user profiles from JSON backup");
      return true;
    } catch (e) {
  ...
  return (
    <DataContext.Provider
      value={{
        ...
        approveLease,
        rejectLease,
        requestLeaseModification,
        approveLeaseModification,
        rejectLeaseModification,
        leaseRenewals,
        deferredPayments,
        createLeaseRenewal,
        updateLeaseRenewal,
