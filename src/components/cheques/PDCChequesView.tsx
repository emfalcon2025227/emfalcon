  ...
  const handleConfirmOperation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeOperation === "DEPOSIT") {
        if (!opProofUrl && !opReference.trim()) {
          setOpModalError(
            isAr
              ? "يجب إرفاق إثبات الإيداع أو إدخال رقم الحافظة."
              : "Please attach deposit proof or enter deposit slip number."
          );
      ...
      setActionNotice({
        type: "error",
        message: err.message || (isAr ? "حدث خطأ أثناء تنفيذ عملية الشيك" : "An error occurred during cheque operation execution"),
      });
