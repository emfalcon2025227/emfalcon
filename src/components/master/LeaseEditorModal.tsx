...
export const LeaseEditorModal: React.FC<LeaseEditorModalProps> = ({
  ...
  const [contractStatus, setContractStatus] = useState<Lease["contractStatus"]>("BINDING");
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    // Prepare Staged Admin Fees Configuration (No financial records created before approval)
    const stagedFees: StagedAdminFeesConfig | undefined = includeAdminFees ? {
      includeAdminFees: true,
      ownerFeeEnabled,
      ownerFeeBasis,
      ownerFeeRate: Number(ownerFeeRate || 0),
      ownerFeeFixed: Number(ownerFeeFixed || 0),
      ownerFeeDueDate: ownerFeeDueDate || startDate,
      ownerFeeImmediateCollection,
      ownerFeePaymentMethod: ownerFeePaymentMethod as any,
      ownerFeeReference,
      tenantFeeEnabled,
      tenantFeeBasis,
      tenantFeeRate: Number(tenantFeeRate || 0),
      tenantFeeFixed: Number(tenantFeeFixed || 0),
      tenantFeeDueDate: tenantFeeDueDate || startDate,
      tenantFeeImmediateCollection,
      tenantFeePaymentMethod: tenantFeePaymentMethod as any,
      tenantFeeReference,
    } : undefined;
    ...
    if (editingLease) {
      if (editingLease.contractStatus === "ACTIVE") {
        // SECTION 11: EDITING EXISTING APPROVED CONTRACTS
        // Check for material financial changes. An active contract cannot be silently changed directly!
        const isRentChanged = parsedRent !== editingLease.annualRent;
        const isDepositChanged = parsedDeposit !== (editingLease.securityDeposit || 0);
        const isDatesChanged = startDate !== editingLease.startDate || endDate !== editingLease.endDate;
        const isFreqChanged = paymentFrequency !== editingLease.paymentFrequency;
        const isInstallmentsChanged = installments.length !== (editingLease.installments || []).length ||
          installments.some((inst, i) => {
            const orig = (editingLease.installments || [])[i];
            return !orig || orig.amount !== inst.amount || orig.dueDate !== inst.dueDate || orig.chequeNumber !== inst.chequeNumber;
          });

        if (isRentChanged || isDepositChanged || isDatesChanged || isFreqChanged || isInstallmentsChanged) {
          const modRes = requestLeaseModification(editingLease.id, {
            proposedAnnualRent: parsedRent,
            proposedSecurityDeposit: parsedDeposit,
            proposedStartDate: startDate,
            proposedEndDate: endDate,
            proposedInstallments: installments.map((inst, idx) => ({
            ...
            modificationReason: language === "ar"
              ? "طلب تعديل بنود عقد ساري من واجهة تحرير العقود"
              : "Financial modification request for active lease",
          });

          if (modRes.success) {
            alert(language === "ar"
              ? "تم إرسال طلب تعديل العقد إلى مركز طلبات الاعتماد بنجاح. يبقى العقد الحالي سارياً ببياناته الأصلية لحين اعتماد الإدارة."
              : "Modification request submitted for approval. Active lease remains unchanged until approved.");
          } else {
            alert(modRes.error || "Failed to submit modification request");
          }
          setIsSubmitting(false);
          onClose();
          return;
        } else {
          // Non-financial modifications (ejariNumber, notes, etc.)
        ...
        // Lease is BINDING or PENDING_APPROVAL: can update staged information
        updateLease(editingLease.id, {
          leaseNumber,
          tenantId,
          ...
          leaseExpenses,
          adminFeePolicy,
          stagedAdminFeesConfig: stagedFees,
        });
      }
      savedLeaseId = editingLease.id;
    } else {
      // NEW LEASE CREATION:
      // Mandatory contract lifecycle: CREATE → BINDING → Submit for Approval → PENDING_APPROVAL
      // CRITICAL: NO REAL FINANCIAL SIDE EFFECTS BEFORE APPROVAL!
      ...
      // Advance contract lifecycle: BINDING → PENDING_APPROVAL
      submitLeaseForApproval(created.id);
    }
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 sm:p-5 bg-slate-50/80 border border-slate-200/80 rounded-2xl space-y-4 shadow-2xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-xs sm:text-sm font-bold text-slate-800 mb-1.5">{language === "ar" ? "حالة العقد (إلزامي)" : "Contract Status"} *</label>
                    {!editingLease ? (
                      <div className="w-full px-3.5 py-2.5 bg-amber-50 border border-amber-300 rounded-xl flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-black text-amber-900 font-mono flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
                          BINDING ({language === "ar" ? "ملزم بانتظار الاعتماد" : "Binding - Pending Approval"})
                        </span>
                        <span className="text-[10px] bg-amber-200 text-amber-950 font-black px-2 py-0.5 rounded-full">
                          {language === "ar" ? "مسار إلزامي" : "Mandatory Gate"}
                        </span>
                      </div>
                    ) : (
                      <div className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">
                          {editingLease.contractStatus || "BINDING"}
                        </span>
                        <span className="text-[10px] bg-slate-200 text-slate-700 font-bold px-2 py-0.5 rounded-full">
                          {editingLease.contractStatus === "ACTIVE" ? (language === "ar" ? "عقد ساري معتمد" : "Approved Active") : editingLease.contractStatus}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <span>
                    {isSubmitting
                      ...
                      ? language === "ar"
                        ...
                        : "Save & Update Lease"
                      : language === "ar"
                      ? "حفظ وإرسال للاعتماد (عقد ملزم BINDING)"
                      : "Save & Submit for Approval (BINDING)"}
                  </span>
                </button>
              </div>
            </div>
