...
export const LeasesView: React.FC<LeasesViewProps> = ({ onNavigateToRenewLease }) => {
  ...
  const { leases, properties, units, tenants, archive, addLease, updateLease, renewLease, deleteLease, addArchiveItem, deleteArchiveItem, getNextLeaseNumber, submitLeaseForApproval, leaseRenewals } = useData();
  ...
  const pendingApprovalsCount =
    leases.filter((l) => l.contractStatus === "BINDING" || l.contractStatus === "PENDING_APPROVAL").length +
    (leaseRenewals?.filter((r) => r.status === "PENDING_APPROVAL").length || 0) +
    leases.filter((l) => l.pendingModification?.status === "PENDING_APPROVAL").length;
  ...
  const filteredLeases = leases.filter((lease) => {
    ...
    const matchStatus = (() => {
      ...
      if (selectedStatus === "ACTIVE") {
        ...
        return status === "ACTIVE" || status === "RENEWED" || status === "UNDER_RENEWAL";
      }
      if (selectedStatus === "BINDING") {
        return status === "BINDING";
      }
      if (selectedStatus === "PENDING_APPROVAL") {
        return status === "PENDING_APPROVAL" || status === "BINDING";
      }
  ...
  const executeSaveLease = () => {
    ...
    if (editingLease) {
      updateLease(editingLease.id, {
        ...
        contractStatus: (editingLease.contractStatus || "BINDING") as any,
      });
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 p-1.5 bg-slate-100/80 rounded-2xl w-fit border border-slate-200/80">
        ...
        >
          ...
          <span>{language === "ar" ? "مركز طلبات الاعتماد والتجديد" : "Approval Requests & Renewals"}</span>
          ...
          <div>
            <SearchableSelect
              options={[
                ...
                { id: "BINDING", label: language === "ar" ? "BINDING (عقود ملزمة بانتظار الاعتماد)" : "BINDING (Binding - Pending Approval)" },
                  ...
                  <tr key={lease.id} className={`${rowBg} transition-colors`}>
                    <td className="py-3 px-4 font-bold text-slate-900">
                      <div className="font-mono text-slate-900">
                        <HighlightedText text={lease.leaseNumber} searchQuery={searchTerm} />
                      </div>
                      <div className="text-[10px] text-slate-400 font-mono">
                        {lease.ejariNumber ? (
                          <>
                            Ejari: <HighlightedText text={lease.ejariNumber} searchQuery={searchTerm} />
                          </>
                        ) : (
                          "No Ejari registered"
                        )}
                      ...
                      {tenant ? (
                        <div>
                          <div className="font-bold text-slate-900">
                            <HighlightedText text={language === "ar" ? tenant.nameAr : tenant.nameEn} searchQuery={searchTerm} />
                          </div>
                          <Badge
                            variant={
                    ...
                    <td className="py-3 px-4">
                      <div className="font-medium text-slate-900">
                        <HighlightedText text={prop ? (language === "ar" ? prop.nameAr : prop.nameEn) : "Property"} searchQuery={searchTerm} />
                      </div>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="text-[10px] text-slate-400 font-mono">
                          Unit #<HighlightedText text={unit?.unitNumber} searchQuery={searchTerm} />
                        </span>
                        {unit && (
                          <div 
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-col gap-1.5 items-start">
                        <Badge
                          variant={
                            lease.contractStatus === "BINDING" || lease.contractStatus === "PENDING_APPROVAL"
                              ? "warning"
                              : daysRemaining <= 0 && lease.contractStatus !== "TERMINATED" && lease.contractStatus !== "CANCELLED"
                              ? "danger"
                          ...
                          size="sm"
                        >
                          {lease.contractStatus === "BINDING"
                            ? (language === "ar" ? "ملزم (بانتظار الاعتماد)" : "BINDING")
                            : lease.contractStatus === "PENDING_APPROVAL"
                            ? (language === "ar" ? "قيد الاعتماد" : "PENDING_APPROVAL")
                            : daysRemaining <= 0 && lease.contractStatus !== "TERMINATED" && lease.contractStatus !== "CANCELLED"
                            ? "EXPIRED"
