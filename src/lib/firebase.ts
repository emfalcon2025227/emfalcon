...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  const isPermission = errMessage.toLowerCase().includes("missing or insufficient permissions") ||
  ...
  } else if (isPermission && isUnauthenticated) {
    console.warn('Firestore Auth Notice: Unauthenticated query blocked by security rules. Detail: ', JSON.stringify(errInfo));
  } else {
