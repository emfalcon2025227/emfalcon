...
export const AdminControlPanel: React.FC = () => {
  ...
  const { users, currentUser, createUser, updateUser, updateUserStatus, updateUserRole, resetUserPassword, hasPermission, deleteUser, importUsersBatch, canRemixAndShare } = useAuth();
  ...
  const handleImportUsersFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const records = Array.isArray(parsed) ? parsed : (parsed.users || parsed.userProfiles || [parsed]);
        const result = await importUsersBatch(records);
        if (result.importedCount > 0 || result.updatedCount > 0) {
          showMsg("success", language === "ar" 
            ? `تم استيراد ${result.importedCount} مستخدم جديد وتحديث ${result.updatedCount} مستخدم في قاعدة البيانات (Firestore) بنجاح!` 
            : `Imported ${result.importedCount} new users, updated ${result.updatedCount} existing users in Firestore!`);
        } else if (result.errors.length > 0) {
          showMsg("error", result.errors[0]);
        } else {
          showMsg("error", language === "ar" ? "لم يتم العثور على سجلات مستخدمين جديدة للاستيراد" : "No new user records found");
        }
      } catch (err) {
        showMsg("error", language === "ar" ? "ملف غير صالح أو صيغة البيانات غير صحيحة (JSON)" : "Invalid file or data format");
      }
    };
          ...
          <div className="p-5 border-b border-slate-200 flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <label className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold shadow-xs transition-colors cursor-pointer">
                ...
                <span>{language === "ar" ? "استيراد ملفات/جدول المستخدمين" : "Import Users Data"}</span>
                <input type="file" accept=".json,.csv" onChange={handleImportUsersFile} className="hidden" />
              </label>
              <button
            ...
            <table className="w-full text-start text-xs">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {users.map((u) => {
                  ...
                  const isProtected = u.role === "SYSTEM_OWNER";
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    disabled={userToEdit.role === "SYSTEM_OWNER"}
                    placeholder={language === "ar" ? "اختر الدور..." : "Select role..."}
                    searchPlaceholder={language === "ar" ? "ابحث بالدور..." : "Search role..."}
                  />
                </div>
