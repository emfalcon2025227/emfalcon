...
import { ChequeEnvelopePrintModal } from "../printing/ChequeEnvelopePrintModal";
import { ChequesBundlePrintModal } from "../printing/ChequesBundlePrintModal";
...
export const LeaseWorkspacePage: React.FC<LeaseWorkspaceModalProps> = ({ lease: initialLease, onClose, onOpenRenew }) => {
  ...
  const [isChequesBundleModalOpen, setIsChequesBundleModalOpen] = useState(false);
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12 text-slate-800">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 text-white p-6 rounded-3xl shadow-lg border border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-amber-400 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer border border-amber-500/30"
            title={isAr ? "طباعة ظرف الشيكات المعتمد" : "Print Official Cheque Envelope"}
          >
            <Mail className="w-4 h-4 text-amber-400" />
            <span>{isAr ? "ظرف الشيكات" : "Cheque Envelope"}</span>
          </button>

          <button
            onClick={() => setIsChequesBundleModalOpen(true)}
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-emerald-400 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer border border-emerald-500/30"
            title={isAr ? "إنشاء ملف PDF لكافة شيكات العقد" : "Generate All Active Cheques PDF"}
          >
            <CreditCard className="w-4 h-4 text-emerald-400" />
            <span>{isAr ? "PDF الشيكات" : "Cheques PDF"}</span>
          </button>
      ...
      {/* Cheque Envelope Print Modal */}
      <ChequeEnvelopePrintModal
      ...
      {/* Cheques Bundle PDF Modal */}
      <ChequesBundlePrintModal
        isOpen={isChequesBundleModalOpen}
        onClose={() => setIsChequesBundleModalOpen(false)}
        lease={lease}
        cheques={cheques}
        owner={owner}
        tenant={tenant}
