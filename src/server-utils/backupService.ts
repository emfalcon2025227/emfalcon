...
export function initFirebaseAdmin() {
    ...
    try {
        ...
        if (!rawAccount) {
            console.warn("[Backup] FIREBASE_SERVICE_ACCOUNT credentials missing.");
            return false;
        }
...
async function getDriveService() {
    const rawAccount = process.env.GDRIVE_SERVICE_ACCOUNT_BASE64 || process.env.GDRIVE_SERVICE_ACCOUNT;
    if (rawAccount) {
