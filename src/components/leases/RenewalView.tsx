...
export const RenewalView: React.FC<RenewalViewProps> = ({
  ...
  const handleExecuteRenewal = async () => {
    ...
    try {
      ...
      const finalRenewal = res.renewal;
      ...
      setSuccessMsg(
        isRtl
          ? `تم إرسال طلب تجديد عقد الإيجار بنجاح وبانتظار الاعتماد الإداري! رقم الطلب: #${finalRenewal.renewalNumber}`
          : `Lease renewal request submitted successfully and pending administrative approval! Request #${finalRenewal.renewalNumber}`
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50/80 pb-16">
      ...
      <div className="w-full px-3 sm:px-5 lg:px-6 mt-6 space-y-6">
        ...
        <form onSubmit={handleOpenConfirmModal} className="space-y-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/90 shadow-2xs space-y-4">
            ...
            {/* Workflow Toggles: WhatsApp Alert */}
      ...
      <Modal
        ...
        title={isRtl ? "مراجعة وتأكيد طلب تجديد العقد" : "Review & Confirm Lease Renewal Request"}
        ...
        <div className="space-y-4">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-2.5">
            ...
            <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-600">
              ...
              <div>
                {isRtl ? "مسار الاعتماد:" : "Approval Mode:"}{" "}
                <span className="font-bold text-amber-800">
                  {isRtl ? "بانتظار الاعتماد الإداري المركزي" : "Pending Administrative Approval"}
                </span>
              </div>
            </div>
          </div>
