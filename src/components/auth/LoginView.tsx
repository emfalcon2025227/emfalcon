...
export const LoginView: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const res = await loginWithGoogle(loginMode);
      if (!res.success) {
        setError(res.error || (language === "ar" ? "فشل تسجيل الدخول بواسطة Google" : "Google sign-in failed"));
      }
    } catch {
      setError(language === "ar" ? "تعذر إتمام تسجيل الدخول بواسطة Google" : "Unable to complete Google sign-in");
    } finally {
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 shadow-2xl text-slate-800 border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              ...
              disabled={isLoading || isGoogleLoading}
              className="w-full py-2.5 px-4 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 text-slate-700 font-bold text-xs shadow-xs hover:shadow-sm disabled:opacity-50 transition-all flex items-center justify-center gap-2.5 cursor-pointer"
            >
              {isGoogleLoading ? (
