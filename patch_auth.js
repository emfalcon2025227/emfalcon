...
code = code.replace(
  ...
  `const provisionPortalAccount = async (params: Omit<ProvisionParams, "existingUsers" | "saveUser">) => {
    try {
      ...
      // If clientManaged is true (Firebase Admin not setup locally), fallback to local for dev
      ...
      // Assuming server saves the user in Firestore directly, but if data.user is returned:
      if (data.user) {
        saveUser(data.user);
      }
