...
export const OCR_PROFILES: Record<string, OcrExtractionProfile> = {
  EMIRATES_ID: {
    ...
    backendEndpoint: "/api/ocr/extract-id",
...
export class OCRService {
  ...
  static async extractDocument(
    ...
    if (backendRes?.isTransportFailure || backendRes?.isHtmlResponse || backendRes?.errorType === "TRANSPORT_FAILURE") {
      recordCheckpoint("CHECKPOINT_07", "AI Request Transport Failure (HTML or Route Intercepted)", "FAIL", t07, backendRes.error);
      return {
        success: false,
        documentType: profile.documentType,
        status: "FAILED",
        ...
        metadata: {
          ...
          processingTimeMs: Date.now() - startTime,
          overallConfidence: 0,
          source: "Transport Protection Layer",
          checkpoints,
        },
        fields: {},
        warnings: [],
        errors: [backendRes.error || "Transport failure connecting to OCR backend"],
        error: backendRes.error || "Transport failure connecting to OCR backend",
        errorAr: backendRes.errorAr || "تعذر الاتصال بخدمة OCR المركزية. تم إيقاف المعالجة لأن استجابة الخدمة لم تكن JSON صحيحة.",
